...
WHERE products.units_in_stock < 25 AND products.discontinued <> 1
...
-- Возможно у меня получился другой порядок, поэтому результат не совпадает со скриншотом, но я проверял, добавляя колонку quantity